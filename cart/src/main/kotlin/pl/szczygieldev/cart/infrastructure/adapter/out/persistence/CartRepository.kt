...
internal class CartRepository(val eventStore: EventStore) : Carts {
    private val clientIdCartIdLookupTable = hashMapOf<ClientId, CartId>()
    ...
    override fun save(cart: Cart, version: Int) = runBlocking {
        ...
        clientIdCartIdLookupTable[cart.clientId] = cart.cartId
    }

    override fun findActiveForClient(id: ClientId): Cart? = clientIdCartIdLookupTable[id]?.let {
        findById(it)
    }
}